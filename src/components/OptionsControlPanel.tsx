...
export const OptionsControlPanel: React.FC = () => {
  ...
  return (
    <div className="space-y-4 pb-12">
      ...
      <div className="border border-neutral-900 bg-[#0a0a0d]/40 rounded-2xl overflow-hidden transition-all duration-300">
        <button
          id="category-header-body"
          onClick={() => toggleCategory('body')}
          className="w-full flex items-center justify-between px-5 py-4 text-left font-serif font-bold text-neutral-200 hover:bg-neutral-900/20 transition-all"
        >
          <div className="flex items-center gap-3">
            <span className="text-[10px] font-mono text-amber-500/80 tracking-widest">01 /</span>
            <span className="tracking-widest text-xs uppercase">BODY ARCHITECTURE</span>
          </div>
          {activeCategory === 'body' ? <ChevronUp className="w-4 h-4 text-amber-500/80" /> : <ChevronDown className="w-4 h-4 text-neutral-600" />}
        </button>

        {activeCategory === 'body' && (
          <div className="p-5 border-t border-neutral-900/60 space-y-6">
            {/* Body Shapes */}
            <div>
              <span className="block text-[10px] font-mono font-bold tracking-widest text-neutral-500 mb-3">BODY SILHOUETTE</span>
                ...
                {Object.values(BODY_SHAPES).map((shape) => (
                  <button
                    ...
                    className={`relative p-3.5 text-left rounded-xl border transition-all duration-200 ${
                      config.bodyShape === shape.id
                        ? 'border-amber-500/30 bg-[#0f0f13] text-amber-500 shadow-md scale-[1.01]'
                  ...
                  >
                    <div className="flex justify-between items-start mb-1.5">
                      ...
                      {config.bodyShape === shape.id && <Check className="w-3.5 h-3.5 text-amber-500" />}
                    </div>
            ...
            {/* Body Woods */}
            <div>
              <span className="block text-[10px] font-mono font-bold tracking-widest text-neutral-500 mb-3">BODY CORE WOOD</span>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5">
                {Object.values(BODY_WOODS).map((wood) => (
                  ...
                  >
                    <div className="flex items-center justify-between mb-1.5">
                      ...
                      {config.bodyWood === wood.id && <Check className="w-3.5 h-3.5 text-amber-500" />}
                    </div>
                    <span className="block text-[9px] font-mono tracking-wide opacity-80">
      ...
      <div className="border border-neutral-900 bg-[#0a0a0d]/40 rounded-2xl overflow-hidden transition-all duration-300">
        ...
        >
          <div className="flex items-center gap-3">
            <span className="text-[10px] font-mono text-amber-500/80 tracking-widest">02 /</span>
            <span className="tracking-widest text-xs uppercase">LACQUER & COATINGS</span>
          </div>
          {activeCategory === 'finish' ? <ChevronUp className="w-4 h-4 text-amber-500/80" /> : <ChevronDown className="w-4 h-4 text-neutral-600" />}
        </button>

        {activeCategory === 'finish' && (
          <div className="p-5 border-t border-neutral-900/60">
      ...
      <div className="border border-neutral-900 bg-[#0a0a0d]/40 rounded-2xl overflow-hidden transition-all duration-300">
        ...
        >
          <div className="flex items-center gap-3">
            <span className="text-[10px] font-mono text-amber-500/80 tracking-widest">03 /</span>
            <span className="tracking-widest text-xs uppercase">NECK & FRETS</span>
          </div>
          {activeCategory === 'neck' ? <ChevronUp className="w-4 h-4 text-amber-500/80" /> : <ChevronDown className="w-4 h-4 text-neutral-600" />}
        </button>
        ...
        {activeCategory === 'neck' && (
          <div className="p-5 border-t border-neutral-900/60 space-y-6">
            {/* Neck Wood */}
            <div>
              <span className="block text-[10px] font-mono font-bold tracking-widest text-neutral-500 mb-3">NECK SHAFT WOOD</span>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5">
                {Object.values(NECK_WOODS).map((wood) => (
                  ...
                  >
                    <div className="flex items-center justify-between mb-1.5">
                      ...
                      {config.neckWood === wood.id && <Check className="w-3.5 h-3.5 text-amber-500" />}
                    </div>
                    <span className="block text-[9px] font-mono tracking-wide opacity-80">
                      {wood.priceDelta === 0 ? 'STANDARD' : `+$${wood.priceDelta}`}
                    </span>
            ...
            {/* Fretboard Wood */}
            <div>
              <span className="block text-[10px] font-mono font-bold tracking-widest text-neutral-500 mb-3">FRETBOARD WOOD</span>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5">
                {Object.values(FRETBOARD_WOODS).map((wood) => (
                  ...
                  >
                    <div className="flex items-center justify-between mb-1.5">
                      ...
                      {config.fretboardWood === wood.id && <Check className="w-3.5 h-3.5 text-amber-500" />}
                    </div>
                    <span className="block text-[9px] font-mono tracking-wide opacity-80">
                      {wood.priceDelta === 0 ? 'STANDARD' : `+$${wood.priceDelta}`}
                    </span>
      ...
      <div className="border border-neutral-900 bg-[#0a0a0d]/40 rounded-2xl overflow-hidden transition-all duration-300">
        ...
        >
          <div className="flex items-center gap-3">
            <span className="text-[10px] font-mono text-amber-500/80 tracking-widest">04 /</span>
            <span className="tracking-widest text-xs uppercase">ELECTRONICS & TRANSDUCERS</span>
          </div>
          {activeCategory === 'pickups' ? <ChevronUp className="w-4 h-4 text-amber-500/80" /> : <ChevronDown className="w-4 h-4 text-neutral-600" />}
        </button>
              ...
              {Object.values(PICKUPS_LAYOUTS).map((layout) => (
                ...
                >
                  <div className="flex justify-between items-start mb-1.5">
                    ...
                    {config.pickupsLayout === layout.id && <Check className="w-3.5 h-3.5 text-amber-500" />}
                  </div>
      ...
      <div className="border border-neutral-900 bg-[#0a0a0d]/40 rounded-2xl overflow-hidden transition-all duration-300">
        ...
        >
          <div className="flex items-center gap-3">
            <span className="text-[10px] font-mono text-amber-500/80 tracking-widest">05 /</span>
            <span className="tracking-widest text-xs uppercase">PLATINGS & ACCENTUATION</span>
          </div>
          {activeCategory === 'hardware' ? <ChevronUp className="w-4 h-4 text-amber-500/80" /> : <ChevronDown className="w-4 h-4 text-neutral-600" />}
        </button>
        ...
        {activeCategory === 'hardware' && (
          <div className="p-5 border-t border-neutral-900/60 space-y-6">
            {/* Hardware color */}
            <div>
              <span className="block text-[10px] font-mono font-bold tracking-widest text-neutral-500 mb-3">HARDWARE METALLIC PLATING</span>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5">
                {Object.values(HARDWARE_COLORS).map((color) => (
                  ...
                  >
                    <div className="flex items-center justify-between mb-1.5">
                      <div className="flex items-center gap-1.5">
                        <span
                          className="w-3 h-3 rounded-full border border-neutral-800 shadow-inner"
                          style={{ backgroundColor: color.hex }}
                        />
                        <span className="text-xs font-bold tracking-wide">{color.name.split(' (')[0].replace(' Finish', '').replace(' Plated', '')}</span>
                      </div>
                      {config.hardwareColor === color.id && <Check className="w-3.5 h-3.5 text-amber-500" />}
                    </div>
                    <span className="block text-[9px] font-mono tracking-wide opacity-80">
            ...
            <div>
              <div className="flex items-center gap-1 mb-3">
                <span className="block text-[10px] font-mono font-bold tracking-widest text-neutral-500">PROTECTIVE PICKGUARD STYLE</span>
                  ...
                  >
                    <div className="flex items-center justify-between mb-1">
                      <div className="flex items-center gap-1.5">
                        {style.color && (
                          <span
                            className="w-2.5 h-2.5 rounded-sm border border-black/10 shadow-inner"
                            style={{ backgroundColor: style.color }}
                          />
                        )}
                        <span className="text-xs font-bold tracking-wide">{style.name.replace(' (Bare Top)', '')}</span>
                      </div>
                      {config.pickguardStyle === style.id && <Check className="w-3.5 h-3.5 text-amber-500" />}
                    </div>
                    <p className="text-[10px] opacity-50 leading-snug truncate">{style.description}</p>
                  </button>
                ))}
              </div>
            </div>
